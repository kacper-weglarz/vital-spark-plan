...
import { MealEntry, MealType, MEAL_LABELS, MEAL_ICONS, FoodItem, UserGoals } from '@/lib/store';
...
export default function MealsPage({ entries, dailyTotals, goals, onAdd, onRemove }: MealsPageProps) {
  ...
  const getMealCalories = (type: MealType) => {
    return todayEntries
      ...
      .reduce((sum, e) => sum + (e.foodItem.calories * e.quantity / 100), 0);
  };
  ...
  const handleAdd = (foodItem: FoodItem, quantity: number, mealType: MealType) => {
    onAdd(foodItem, quantity, mealType);
  };
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="px-4 pt-2 pb-28">
      ...
      {/* Meals */}
      {mealTypes.map(type => {
        const mealEntries = todayEntries.filter(e => e.mealType === type);
          ...
          <motion.div key={type} variants={item} className="ios-card p-4 mb-3">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="w-8 h-8 bg-primary/10 rounded-xl flex items-center justify-center active:scale-90 transition-transform"
              >
            ...
            {mealEntries.length > 0 ? (
              <div className="space-y-2">
                {mealEntries.map(entry => {
                  const mult = entry.quantity / 100;
                  return (
                    <div key={entry.id} className="flex items-center gap-3 p-2.5 bg-muted/50 rounded-xl">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold truncate">{entry.foodItem.name}</p>
                        <p className="text-xs text-muted-foreground">{entry.quantity}g • {Math.round(entry.foodItem.calories * mult)} kcal</p>
                      </div>
                      <div className="flex gap-2.5 text-[10px] font-bold">
                        <span className="macro-protein">{Math.round(entry.foodItem.protein * mult)}B</span>
                        <span className="macro-carbs">{Math.round(entry.foodItem.carbs * mult)}W</span>
                        <span className="macro-fat">{Math.round(entry.foodItem.fat * mult)}T</span>
                      </div>
