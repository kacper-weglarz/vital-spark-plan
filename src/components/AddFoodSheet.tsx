...
import { MealType, MEAL_LABELS } from '@/lib/store';
...
interface AddFoodSheetProps {
  ...
  onClose: () => void;
  mealType: MealType;
  onAdd: (product: any, quantity: number, mealType: MealType) => void;
}
      ...
      {isOpen && (
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 bg-foreground/20 z-50" onClick={onClose} />
          <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
            ...
            <div className="p-4 border-b border-border">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={onClose} className="p-1 rounded-full bg-muted"><X className="w-5 h-5 text-muted-foreground" /></button>
              </div>
              <div className="flex gap-2">
                ...
                <button className="p-2.5 bg-primary/10 rounded-xl"><ScanBarcode className="w-5 h-5 text-primary" /></button>
              </div>
            </div>
            {selectedFood ? (
              <div className="p-4 flex-1">
                ...
                <div className="grid grid-cols-4 gap-2 text-center mb-4 ios-card p-3">
                  <div><p className="text-sm font-bold text-primary">{Math.round(selectedFood.calories * Number(quantity) / 100)}</p><p className="text-[10px] text-muted-foreground">kcal</p></div>
                  <div><p className="text-sm font-bold macro-protein">{Math.round(selectedFood.protein * Number(quantity) / 100)}</p><p className="text-[10px] text-muted-foreground">białko</p></div>
                  <div><p className="text-sm font-bold macro-carbs">{Math.round(selectedFood.carbs * Number(quantity) / 100)}</p><p className="text-[10px] text-muted-foreground">węgle</p></div>
                  <div><p className="text-sm font-bold macro-fat">{Math.round(selectedFood.fat * Number(quantity) / 100)}</p><p className="text-[10px] text-muted-foreground">tłuszcz</p></div>
                </div>
                <button onClick={handleAdd} className="w-full py-3.5 bg-primary rounded-2xl text-primary-foreground font-bold text-base">Dodaj produkt</button>
              </div>
            ) : (
